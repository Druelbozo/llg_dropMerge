...
USAGE:
    ...
    Examples:
    # Sync specific paths
    python scripts/aws/s3/sync_from_s3.py /src/config/themes
    python scripts/aws/s3/sync_from_s3.py /index.html
    python scripts/aws/s3/sync_from_s3.py /src/config/themes /css /index.html
    ...
    python scripts/aws/s3/sync_from_s3.py /src/config/themes --dry-run
    
    # Force download all files (ignore hash checks)
    python scripts/aws/s3/sync_from_s3.py /src/config/themes --force
"""
...
def parse_project_path(project_root, project_path, bucket_override=None, prefix_override=None):
    ...
    Paths are synced from: llg-games/games/drop-merge/{path}/
    - Files at root (e.g., 'index.html') → games/drop-merge/index.html
    - Directories (e.g., '/src') → games/drop-merge/src/{files}
    ...
    if prefix_override:
        ...
        # Build S3 prefix: games/drop-merge/{project_path}/
...
Examples:
  ...
  # Override bucket or prefix
    python scripts/aws/s3/sync_from_s3.py /src/config/themes --bucket my-bucket
    python scripts/aws/s3/sync_from_s3.py /src/config/themes --prefix custom-prefix/
        """
    )
    parser.add_argument(
        'paths',
