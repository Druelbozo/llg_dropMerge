...
def parse_project_path(project_root, project_path, bucket_override=None, prefix_override=None):
    ...
    Paths are synced to: llg-games/games/drop-merge/{path}/
...
def sync_to_s3(s3_client, bucket_name, local_dir, local_files, s3_objects, s3_prefix, 
    ...
    if not local_files and len(orphaned_keys) > 0:
        ...
        if folders_deleted == 0:
            ...
            # For directories, sync_scope_prefix ends with '/' (e.g., "games/drop-merge/test/")
            # For files, sync_scope_prefix is the full S3 key without trailing '/' (e.g., "games/drop-merge/index2.html")
...
Examples:
  ...
  # Override bucket or prefix
    ...
    parser.add_argument(
        '--production',
        action='store_true',
        help='Deploy production build: use dist/ as root and PRODUCTION_PATHS (index.html, assets, js, src/config).'
    )
    ...
    if args.production:
        args.from_dir = 'dist'
    if args.from_dir:
        project_root = os.path.join(project_root, args.from_dir)
