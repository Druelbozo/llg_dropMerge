 ...
 *   --invalidate-all        Invalidate everything in the S3 prefix folder (e.g., /games/drop-merge/*)
...
const DEFAULT_S3_PREFIX = 'games/drop-merge/';
...
function normalizeInvalidationPaths(paths, s3Prefix) {
  ...
  // e.g., 'games/drop-merge/' -> '/games/drop-merge/'
  ...
  const normalized = paths.map((p) => {
    ...
    // Build the full CloudFront path: /games/drop-merge/{path}/*
...
function getInvalidateAllPath(s3Prefix) {
  // Convert S3 prefix (e.g., 'games/drop-merge/') to CloudFront path (e.g., '/games/drop-merge/*')
